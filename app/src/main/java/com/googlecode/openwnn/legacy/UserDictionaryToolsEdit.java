...
public abstract class UserDictionaryToolsEdit extends Activity implements View.OnClickListener {
	/**
	 * The class information for intent(Set this informations in the extend
	 * class)
	 */
	...
	/**
	 * The constant for notifying dialog (The length of specified stroke or
	 * candidate exceeds the limit)
	 */
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		getWindow().setFeatureInt(Window.FEATURE_CUSTOM_TITLE, R.layout.user_dictionary_tools_edit_header);
	...
	/**
	 * Change the state of the "Add" button into the depending state of input
	 * area.
	 */
	public void setAddButtonControl() {
		...
		mReadEditText.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
				...
				if ((mReadEditText.getText().toString().length() != 0) && (mCandidateEditText.getText().toString().length() != 0)) {
					mEntryButton.setEnabled(true);
				} else {
					mEntryButton.setEnabled(false);
				}
	...
	protected Dialog onCreateDialog(int id) {
		...
		case DIALOG_CONTROL_WORDS_DUPLICATE:
			...
			return new AlertDialog.Builder(UserDictionaryToolsEdit.this).setIcon(android.R.drawable.ic_dialog_alert)
	...
	private boolean addDictionary(String stroke, String candidate) {
		...
		/* add word event */
		OpenWnnEvent event = new OpenWnnEvent(OpenWnnEvent.ADD_WORD, WnnEngine.DICTIONARY_TYPE_USER, wnnWordAdd);
